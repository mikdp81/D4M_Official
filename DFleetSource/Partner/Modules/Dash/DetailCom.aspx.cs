    ...
    public partial class DetailCom : System.Web.UI.Page
    {
        ...
        protected void btnInserisci_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    //aggiorna lo status di tutte le comunicazioni correlate
                    servizioComunicazioni.UpdateRiAperturaComunicazione(UidcomunicazionePadre, Uidtenant);
                    ...
                    if (data != null)
                    {
                        ...
                        {
                            MailHelper.SendMail("", servizioAccount.DetailId(new Guid(hdMittente.Value)).Email, "", "", "", "", "Help Desk - Risposta Ticket", servizioUtility.InsComEmail((Guid)Membership.GetUser().ProviderUserKey, UidcomunicazionePadre, testo, dataTemplate.Corpo), "", datiUtente.ReturnObjectTenant());
                        }
                        ...
                        for (int i = 0; i < uploadedFiles.Count; i++)
                        {
                            ...
                            {
                                errorefile += ex.Message + " - " + userPostedFile.FileName + "<br />";
                                operazioneok += "0";
                            }
                        }
        ...
        public string ReturnUtente(string cognome, string uidmittente)
        {
            string retVal;

            if ((Guid)Membership.GetUser().ProviderUserKey == SeoHelper.GuidString(uidmittente))
            {
                retVal = cognome;
            }
            else
