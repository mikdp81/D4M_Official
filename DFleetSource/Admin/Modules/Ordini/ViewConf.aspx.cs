...
namespace DFleet.Admin.Modules.Ordini
{
    public partial class ViewConf : System.Web.UI.Page
    {
            ...
            if (!Page.IsPostBack)
            {
                if (Guid.TryParse(SeoHelper.EncodeString(Request.QueryString["uid"]), out Guid uid))
                {
                    ...
                    if (data != null)
                    {
                        ...
                        switch (data.Idstatusordine)
                        {
                            case 1: 
                                btnAutorizza.Visible = true;
                                btnNonAutorizza.Visible = true;
                                break;
                            case 10:
                                btnAutorizza.Visible = false;
                                btnNonAutorizza.Visible = false;
                                break;
                            case 20: 
                                btnAutorizza.Visible = false;
                                btnNonAutorizza.Visible = false;
        ...
        protected void btnAutorizza_Click(object sender, EventArgs e)
        {
            Response.Redirect("EditConferma-" + SeoHelper.EncodeString(hduid.Value));
        }

        protected void btnNonAutorizza_Click(object sender, EventArgs e)
        {
            Response.Redirect("EditScarta-" + SeoHelper.EncodeString(hduid.Value));
        }
        public string UrlCreaPdf()
        {
            return "CreaPdfConf-" + SeoHelper.EncodeString(hduid.Value);
        }
    }
}
