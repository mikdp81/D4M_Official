...
// ***********************************************************************
// <copyright file="ViewFileAuto.aspx.cs" company="">
//     . All rights reserved.
// </copyright>
// <summary></summary>
// ***********************************************************************
...
{
    public partial class ViewFileAuto : System.Web.UI.Page
    {
        ...
        public void loadPage()
        {
            ...
            int totaleRighe = servizioContratti.SelectCountFileAuto(targa, codsocieta, UserId, Uidtenant);
            ...
            if (gvFileAuto.HeaderRow != null)
            {
                //aggiunge il tag THEAD nell'intestazione della gridview
                gvFileAuto.HeaderRow.TableSection = TableRowSection.TableHeader;
            }
            ...
            lblNumRecord.Text = "Auto: " + HttpUtility.HtmlEncode(totaleRighe);
            if (totaleRighe == 0)
            {
                lblMessage.Text = "Nessun dato disponibile. Ricerca con altri parametri.";
                pnlMessage.Visible = true;
        ...
        {
            Response.Redirect("ViewFileAuto");
        }
        ...
        public string ReturnFileAssicurazioni(string targa)
        {
            string retVal = "";
            IContrattiBL servizioContratti = new ContrattiBL();

            List<IContratti> dataDoc = servizioContratti.SelectDocumentiAuto(targa);
            if (dataDoc != null && dataDoc.Count > 0)
            {
                foreach (IContratti resultDoc in dataDoc)
                {
                    retVal += "Anno: " + resultDoc.Anno + " - <a href=\"../../../DownloadFile?type=contratti&nomefile=" + resultDoc.Nomefile + "\" target='_blank'><i class='icon-check' data-toggle='tooltip' title='' data-original-title='Apri'></i></a><br />";
                }
            }
