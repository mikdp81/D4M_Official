...
namespace DFleet.Users.Modules.Ordini
{
    public partial class ViewConf : System.Web.UI.Page
    {
        ...
        protected void btnModifica_Click(object sender, EventArgs e)
        {
            ...
            Guid UserId = (Guid)Membership.GetUser().ProviderUserKey;
            string grade = "";
            decimal maxdeltacanone = 0;

            IContratti data = servizioContratti.DetailOrdiniId(Uid);
            if (data != null)
            {
                importodeltacanone = data.Deltacanone;
            }
            ...
            switch (grade)
            {
                ...
                case "27":
                case "30":
                    maxdeltacanone = SeoHelper.MaxDeltaCanoneSen();
                    break;

                case "25": //SMANAGER
                    maxdeltacanone = SeoHelper.MaxDeltaCanoneMan();
                    break;

                case "17": //DIRECTOR
                    maxdeltacanone = SeoHelper.MaxDeltaCanoneSMan();
                    break;
            }



            if (maxdeltacanone > importodeltacanone)
            {
                idstatusordine = 10; //già autorizzato
            }
            else
            {
                idstatusordine = 1; //configurato
            }

            if (importodeltacanone == 0) 
            {
                idstatusordine = 10; //se optonal canone = 0 va in status Conferma ordine in firma (N.B. rimettere 40 per velocizzare processo)
            }

            if (servizioContratti.UpdateChangeStatusOrdine(Uid, idstatusordine, "", SeoHelper.ReturnSessionTenant()) == 1)
            {
                ILogBL log = new LogBL();
                log.InsLog(Page.TemplateSourceDirectory + "/" + Page.Title, "Ordine Inviato " + Uid);
